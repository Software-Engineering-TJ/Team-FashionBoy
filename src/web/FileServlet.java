...
//                            System.out.println(req.getRequestURL());
     ...
     * @throws IOException
     */
    public void ossUploadFile(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        System.out.println("开始测试oss上传文件");
        //1.先判断是否是多段数据,不是多段数据的话一定不是文件上传，后续操作无法展开
        if (ServletFileUpload.isMultipartContent(req)) {
            //2.创建FileItemFactory实现类
            FileItemFactory fileItemFactory = new DiskFileItemFactory();
            ...
            try {
                ...
                System.out.println("文件上传到了：" + fileUrl);
                map.put("result", 1);
                resp.getWriter().write(gson.toJson(map));
            } catch (FileUploadException e) {
                e.printStackTrace();
     ...
     * @throws IOException
     */
    public void ossDownloadFile(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        System.out.println("开始测试oss下载文件");
//        //1.获取文件url
//        String reqJson = RequestJsonUtils.getJson(req);
//        Map<String, String> reqObject = gson.fromJson(reqJson, new TypeToken<Map<String, String>>() {
//        }.getType());
...
//        String fileUrl = reqObject.get("fileUrl");
        String fileUrl = "https://hjk-files.oss-cn-shenzhen.aliyuncs.com/1/1/20040/CCF CSP第24程序设计认证考试（2021.12.05）报名通知（同济大学考点）.pdf";
